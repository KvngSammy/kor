...
app.post('/api/investments', authenticate, (req, res) => {
  try {
    ...
    res.json({ investmentId: result.lastInsertRowid, transactionRef, totalAmount, message: 'Investment initiated. Complete payment to finalize.' });
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/api/investments/:id/verify-payment', authenticate, (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, ?)')
      .run(req.user.id, 'Investment Confirmed', `You have successfully invested N${investment.total_amount.toLocaleString()} in ${property.title}`, 'success');
...
app.post('/api/dividends/:id/execute', authenticate, requireRole('admin', 'company'), (req, res) => {
  try {
    ...
    for (const payout of payouts as any[]) {
      try {
        ...
        if (success) {
          ...
          db.prepare('INSERT INTO notifications (user_id, title, message, type) VALUES (?, ?, ?, ?)')
            .run(payout.user_id, 'Dividend Paid', `You received N${payout.amount.toLocaleString()} dividend payout`, 'success');
          successCount++;
        } else {
    ...
    res.json({ message: 'Bulk dividend execution completed', totalProcessed: payouts.length, successCount, failCount, status: finalStatus });
  } catch (err: any) {
    res.status(400).json({ error: err.message });
  }
});
...
app.post('/api/admin/properties', authenticate, requireRole('admin', 'company'), upload.array('images', 10), (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO communities (property_id, name, description) VALUES (?, ?, ?)')
      .run(result.lastInsertRowid, `${title} Investors`, `Community for ${title} investors`);
...
app.get('/api/seed', (req, res) => {
  try {
    ...
    const admin = db.prepare('INSERT OR IGNORE INTO users (email, password, full_name, role, kyc_status) VALUES (?, ?, ?, ?, ?)')
      .run('admin@kor.ng', adminPass, 'System Administrator', 'admin', 'verified');
    ...
    const investors = [
      { email: 'chinedu@kor.ng', name: 'Chinedu Okafor', phone: '08031234567' },
      { email: 'fatima@kor.ng', name: 'Fatima Abdullahi', phone: '08042345678' },
      { email: 'emeka@kor.ng', name: 'Emeka Nwosu', phone: '08053456789' },
      { email: 'amina@kor.ng', name: 'Amina Ibrahim', phone: '08064567890' },
      { email: 'tolu@kor.ng', name: 'Tolu Adeyemi', phone: '08075678901' },
    ];
    for (const inv of investors) {
      const pass = bcrypt.hashSync('password123', 10);
      db.prepare('INSERT OR IGNORE INTO users (email, password, full_name, phone, role, kyc_status) VALUES (?, ?, ?, ?, ?, ?)')
...
app.listen(PORT, () => {
  console.log(`Kor Nigeria API running on port ${PORT}`);
});
